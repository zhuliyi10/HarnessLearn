...
1. `后台运行 sleep 5 && echo '构建完成'，等待期间帮我写一个 notes.md 记录今天的工作，然后看看后台任务结果`
2. `后台跑 sleep 3 && echo 任务A，再跑 sleep 6 && echo 任务B，都启动后先别等，告诉我现在有几个后台任务`
3. `直接运行 sleep 3`（看它走哪个工具）

**观察重点**：模型启动后台任务 → 先干别的 → 收到紫色的 `[bg] 已注入 N 条完成通知` → 处理结果。

- Prompt 1：完整走一遍"后台执行 + 并发工作 + 通知驱动"。注意完成通知不是模型去轮询的，而是 harness 在下一轮注入进 messages 的。
- Prompt 2：两个后台任务交错完成，观察 `check_job` 拿到的是哪一条，以及模型如何区分两个任务的结果。
- Prompt 3：`bash` 适合 10 秒内的快命令，慢命令应该走 `bash_background`——观察模型怎么选。选错走同步 bash，循环就会被 sleep 卡住。
