...
1. `帮我写一条 commit message：我修好了登录页面点击无反应的 bug`
2. `按项目规范审查一下 s01_agent_loop/code.py 的 Python 代码风格`
3. `1 + 1 等于几？`

**观察重点**：蓝色的 `[skill] 已加载技能: xxx`——技能按需加载，只在需要时全文注入上下文。

- Prompt 1：模型先 `load_skill("git-commit")`，再按技能里的规范产出提交信息。对比不加载技能时的输出，体会"技能 = 给模型的领域 SOP"。
- Prompt 2：`python-style` 技能加载后，模型还会自己 `read_file` 去读目标代码——技能是知识，工具是手脚，两者配合干活。
- Prompt 3：和技能无关的问题。观察它不加载任何技能直接回答——按需加载，不白占上下文。
